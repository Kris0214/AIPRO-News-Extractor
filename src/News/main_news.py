...
def main():
    ...
    try:
        ...
        # 7. Initialize news LLM service (specific)  # ← 新增
        logger.info("Initializing News LLM service...")
        news_llm_service = NewsLLMService(llm_service)

        # 7. Initialize news service
        news_service = NewsService(
            db_manager=db_manager,
            news_llm_service=news_llm_service,
            config=config['news']
        )
